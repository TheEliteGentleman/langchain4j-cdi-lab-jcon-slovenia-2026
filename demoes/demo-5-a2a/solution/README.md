...
Wait for all three to print `Liberty is running in dev mode.` before testing.
...
```bash
curl http://localhost:9080/.well-known/agent-card.json | jq .
curl http://localhost:9081/.well-known/agent-card.json | jq .
```
...
```bash
curl "http://localhost:9082/api/styled-story?topic=dragons+and+wizards&style=comedy" | jq .
```
