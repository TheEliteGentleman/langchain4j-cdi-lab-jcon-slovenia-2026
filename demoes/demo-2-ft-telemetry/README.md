...
Second JCON Slovenia demo: adding **resilience** (MicroProfile Fault Tolerance) and **observability** (OpenTelemetry) to a LangChain4j-CDI AI agent that already uses Memory, RAG, and Tools.
...
```bash
# Option B: Mistral AI (remote) for Unix/Linux/MacOS
export MISTRAL_API_KEY=your-key-here
```

```
# Option B: Mistral AI (remote) for Windows
...
The app is available at **http://localhost:9080/**.
...
curl -X POST -H "Content-Type: text/plain" \
  ...
  -d "Which expeditions are available?" \
