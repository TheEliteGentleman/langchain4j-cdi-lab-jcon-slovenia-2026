...
/**
 * AI Agent embodying Ragnar the Skald, the Jarl presiding over the Hnefatafl game at the Grand Thing. 
 *
 * <p>Registered via {@code @RegisterAIService} with:
 * <ul>
 *   <li>{@code chatModelName = "mistral"} - local Ollama model (mistral-3b).</li>
 *   <li>{@code toolProviderName = "mcp"} - MCP server exposing the {@code roll} tool
 *       for rolling N six-sided runestones.</li>
 *   <li>{@code chatMemoryProviderName = "my-memory"} - {@link ChatMemoryProviderBean},
 *       retaining the last two exchanges to compare the current roll against the previous one.</li>
 * </ul>
 *
 * <p>The agent is strictly required to use the {@code roll} tool for every roll
 * and must never fabricate results. 
*/
...
public interface HnefataflJarlAI {
    ...
    @SystemMessage("""
        ...
        RUNES: [1, 1]
        TOTAL: 2
        FATE: Eyes of the serpent! Curse of the Norns -- the warrior loses!
        ...
        RUNES: [2, 6]
        TOTAL: 8
        FATE: Rune reached! The warrior wins!
        ...
        2. If a rune is marked and the warrior says "Re-roll" or "Continue"
        ...
        - Expressions: "Skal!", "For Odin!", "Eyes of the serpent!", "Ragnarök!", "The Warrior triumphs!"
        - Respond in English
