...
Open http://localhost:9080/demo-1/ and send a few messages:
...
curl -X POST -H "Content-Type: text/plain" \
  ...
  http://localhost:9080/demo-1/api/chat
```

### 4. Test streaming

Open http://localhost:9080/demo-1/stream.html and type a message — tokens appear in real time!

### 5. Test image analysis (requires vision model)

Open http://localhost:9080/demo-1/image.html and upload an image.
...
```bash
# In the app server terminal: Ctrl+C
```
...
- Check what's using it: `lsof -i :8080` (*nix, MacOS) or `netstat -ano | find "8080"` (Windows)
