...
/**
 * A CDI-managed {@link ChatMemoryProvider} that creates a {@link LastDiceRollChatMemory}
 ...
 * Each session retains the last two roll exchanges, allowing the model
 * to compare the current result with the previous one without accumulating obsolete history.
 */
@ApplicationScoped
@Named("my-memory")
public class ChatMemoryProviderBean implements ChatMemoryProvider {
    ...
    /**
     * Returns the current list of messages for a session (for debugging purposes only).
     *
     * @param sessionId The memory identifier used during the creation of the session.
