# LangChain4j-CDI — JCON Slovenia 2026 Demos
...
- Open `demoes/` as a Maven project
...
6. `mvn clean install` then `mvn liberty:dev -e` 
7. Test:
```bash
...
curl -X POST http://localhost:9080/api/chat \
  -H "Content-Type: text/plain" \
  -d "What sessions are at JCON Slovenia?"
```
...
9. Show the OpenTelemetry traces in the logs
...
5. Deploy and test:
```bash
curl -X POST http://localhost:9080/api/conference/query \
  -H "Content-Type: text/plain" \
  -d "What conferences are at JCON Slovenia?"
```
...
- [ ] OpenLiberty / WildFly 39 ready (or the plugin downloads it)
- [ ] IntelliJ or similar IDE (that supports Jakarta EE annotations) open on `demoes/` folder
- [ ] Terminal ready for curl commands (use GitBash if OS is Windows)
- [ ] Solutions tested the day before (`solution/` of each demo)
- [ ] iPad in Sidecar for speaker notes
- [ ] Reveal.js slides launched (open `slides/index.html` in the browser)

## In case of emergency

```bash
# Backup solution for each demo (Linux / macOS):
cd demo-1-ai-agent/solution && mvn clean install && mvn liberty:dev-e
cd demo-2-ft-telemetry/solution && mvn clean install && mvn liberty:dev-e
cd demo-3-mcp/solution && mvn clean install && mvn liberty:dev-e
